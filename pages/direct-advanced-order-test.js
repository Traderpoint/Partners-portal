...
export default function DirectAdvancedOrderTest() {
  ...
  const handleInputChange = (field, value) => {
    if (field.startsWith('addons.')) {
      ...
      // Auto-update product name and price
      if (field === 'productId') {
        switch (value) {
          case '1':
  ...
  const createDirectAdvancedOrder = async () => {
    setLoading(true);
    setError(null);
    setResult(null);

    try {
      console.log('🚀 Creating direct advanced order...');
      ...
      console.log('📤 Direct advanced order data:', orderData);

      const response = await fetch('/api/hostbill/create-advanced-order', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        setResult(data);
        console.log('✅ Direct advanced order created:', data);
      } else {
        setError(data.error || 'Order creation failed');
        console.error('❌ Direct advanced order failed:', data);
      }
    } catch (err) {
      setError(err.message);
      console.error('❌ Error creating direct advanced order:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      <Head>
        <title>Direct Advanced Order Test</title>
      </Head>

      <h1>⚡ Direct Advanced Order Test</h1>
      <p>Test pokročilého vytvoření objednávky přímo v HostBill (bez middleware)</p>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '14px', color: '#666', marginTop: '5px' }}>
          Direct HostBill API (no middleware)
        </div>
      </div>

      <button
        onClick={createDirectAdvancedOrder}
        disabled={loading}
        style={{
          padding: '15px 30px',
      ...
      >
        {loading ? '⏳ Creating Order...' : '⚡ Create Direct Advanced Order'}
      </button>
      ...
      {result && (
        <div style={{ backgroundColor: '#d4edda', border: '1px solid #c3e6cb', borderRadius: '8px', padding: '20px' }}>
          <h3 style={{ margin: '0 0 15px 0', color: '#155724' }}>✅ Direct Advanced Order Created</h3>
          ...
          {result.summary && (
            <div style={{ backgroundColor: 'white', border: '1px solid #c3e6cb', borderRadius: '6px', padding: '15px', marginBottom: '15px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px' }}>
                ...
                <div><strong>Orders Created:</strong> {result.orders_created}</div>
